...
const Header = ({
                    ...
                    adminData, // Données initiales (optionnel)
                    userRole = "admin",
                }) => {
    ...
    // État local pour les données utilisateur (synchronisé)
    const [userData, setUserData] = useState(adminData || {});

    // --- 1. FONCTION DE RÉCUPÉRATION DES DONNÉES ---
    // Mémorisé avec useCallback pour être appelé n'importe quand
    const fetchUserProfile = useCallback(async () => {
        try {
            const response = await API.get("/profile");
            if (response.data.success) {
                setUserData(response.data.data);
                // On force aussi le rafraîchissement de l'avatar via le timestamp
                setAvatarVersion(prev => prev + 1);
            }
        } catch (error) {
            console.error("Erreur sync header:", error);
        }
    }, []);

    // --- 2. INITIALISATION ET ÉCOUTEUR D'ÉVÉNEMENT (TEMPS RÉEL) ---
    useEffect(() => {
        // Chargement initial si pas de données ou pour être sûr d'être à jour
        fetchUserProfile();

        // Écoute l'événement global "profileUpdated" déclenché par Profile.jsx
        const handleProfileUpdate = () => {
            console.log("🔄 Header: Mise à jour du profil détectée !");
            fetchUserProfile();
        };
    ...
    // Construction du Nom Complet - MODIFIÉ ICI
    const getFullName = () => {
        // PRIORITÉ 1 : Utiliser le nom d'affichage (name) s'il existe
        if (userData.name && userData.name.trim() !== "") {
            return userData.name.trim();
        }

        // PRIORITÉ 2 : Sinon, utiliser nom + prénom
    ...
    // Rôle effectif
    const effectiveRole = (userData.role || userData.formattedrole || userRole || "admin").toLowerCase();
    ...
    // Chargement des notifications (inchangé)
    useEffect(() => {
    ...
    const getAvatarUrl = (path) => {
        ...
        return `${base}${url}?v=${avatarVersion}&t=${Date.now()}`;
    };
                                    ...
                                    <div className={`px-6 py-4 ${currentRole.light} border-b ${currentRole.border} mb-2`}>
                                        <div className="flex items-center space-x-4">
                                            ...
                                            <div className="overflow-hidden">
                                                <p className="font-bold text-gray-800 truncate" title={fullName}>
                                                    {fullName}
                                                </p>
                                                <p className="text-xs text-gray-500 truncate mb-1.5" title={userData.email}>
