...
const PublicEnseignantsView = () => {
    ...
    useEffect(() => {
        const fetchEtablissements = async () => {
            // Ne rien faire si les universités ne sont pas encore chargées
        ...
        fetchEtablissements();
    }, [selectedUniv, universites]); // ✅ Dépendances correctes

    // ✅ Calculer les établissements disponibles (memoized pour éviter recalculs)
    const availableEtablissements = useMemo(() => {
        const etabNames = Array.from(new Set(etablissements.map(e => e.nom))).sort();
        return etabNames;
    }, [etablissements]);
    ...
    // ✅ Charger les enseignants avec filtres (avec debounce pour la recherche)
    useEffect(() => {
        // Ne rien faire si les universités ne sont pas encore chargées
        if (universites.length === 0) return;
        ...
        const fetchEnseignants = async () => {
            ...
            try {
                ...
                // ✅ Transformer les données pour correspondre à l'affichage existant
    ...
    // ✅ Créer la liste des universités pour le select
    const UNIVERSITES_LIST = useMemo(() => {
        return universites.map((u) => u.nom);
    }, [universites]);

    // Logique de Pagination (calcul côté serveur Laravel)
    const totalPages = Math.ceil(totalItems / itemsPerPage) || 1;

    // ✅ S'assurer que la page courante est valide (sans causer de boucle)
    useEffect(() => {
        if (totalPages > 0 && currentPage > totalPages) {
            setCurrentPage(totalPages);
        }
    }, [totalPages]); // ✅ Ne dépend que de totalPages
                        ...
                        {loading ? (
                            ...
                            <>
                                ...
                                <span className="text-[#4c7026]">
                                    {teachers.length > 0 ? (currentPage - 1) * itemsPerPage + 1 : 0}
                                </span>{" "}
                                à{" "}
                                <span className="text-[#4c7026]">
                                    {Math.min(currentPage * itemsPerPage, totalItems)}
                                </span>{" "}
                                sur <span className="text-[#4c7026]">{totalItems}</span> enseignants
                            </>
                        )}
                    </div>
                                ...
                                >
                                    ...
                                    <div className="col-span-12 md:col-span-4 flex items-start gap-3">
                                        ...
                                        <div>
                                            ...
                                            <h3 className="font-bold text-gray-800 text-sm group-hover:text-[#4c7026] transition-colors uppercase leading-tight">
                                                {teacher.nom}
                                            </h3>
                                    ...
                                    <div className="col-span-12 md:col-span-3 pl-14 md:pl-0">
                                        ...
                                        <div className="hidden md:block text-[11px] text-gray-500 ml-6 mt-0.5 truncate">
                                            {teacher.univ}
                                        </div>
                                    </div>
                                    ...
                                    <div className="col-span-6 md:col-span-2 pl-14 md:pl-0 mt-2 md:mt-0">
                                        ...
                                        <span className="inline-flex items-center px-2.5 py-1 rounded bg-gray-100 text-gray-700 text-xs font-medium border border-gray-200">
                                            {teacher.grade}
                                        </span>
                                    </div>

                                    {/* Colonne 4: Diplôme & Spécialité */}
                                    <div className="col-span-6 md:col-span-3 pl-14 md:pl-0 mt-2 md:mt-0">
                                        <span className="md:hidden text-[10px] font-bold text-gray-400 uppercase block mb-1">
                                            Qualification
                                        </span>
                                        <div className="flex flex-col gap-1.5">
                                            <div className="flex items-center gap-2 text-xs font-bold text-[#4c7026]">
