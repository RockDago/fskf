...
const Login = () => {
  ...
  useEffect(() => {
    ...
    const checkExistingSession = async () => {
      const lastCheckTimestamp = Number(
        sessionStorage.getItem("last_check_auth_ts") || 0
      );
      const now = Date.now();

      if (now - lastCheckTimestamp < 3000) {
        setAuthChecked(true);
        return;
      }

      sessionStorage.setItem("last_check_auth_ts", String(now));
  ...
  }, [navigate]); // ✅ Seulement 'navigate' dans les dépendances
      ...
      <div className="bg-white p-6 rounded-xl shadow-lg w-full max-w-sm border border-gray-200">
        <div className="text-center mb-8">
          ...
          <p className="mt-2 text-sm font-medium text-gray-500">
            Portail <span className="text-[#b4cd7b] font-bold">Admin</span> ou{" "}
            <span className="text-[#b4cd7b] font-bold">Agent</span>
          </p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          ...
          <div className="relative group">
            ...
            <div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
              <svg
                className="w-4 h-4 text-gray-400 group-focus-within:text-[#b4cd7b] transition-colors duration-200"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
          ...
          <div className="relative group">
            ...
            >
              {showPassword ? (
                <svg
                  className="w-4 h-4 group-focus-within:text-[#b4cd7b] transition-colors duration-200"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
