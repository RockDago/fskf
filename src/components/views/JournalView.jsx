...
const JournalView = () => {
    ...
    // plus de notion d'accès public côté front
    ...
    // Chargement initial uniquement (plus de setInterval)
    useEffect(() => {
        fetchAllData();
        checkCurrentUser();
    }, []);

    // Recharger les données quand les filtres changent (léger délai)
    useEffect(() => {
    ...
    const checkCurrentUser = async () => {
        try {
            ...
            // Méthode 2 : endpoint dédié check-auth
            try {
    ...
    // Filtrage côté client (corrigé : recherche partielle, insensible à la casse)
    const filteredAuditLog = (auditData.audit_log || []).filter((log) => {
        ...
        const matchUser =
            !filters.user ||
            logUser.includes(filters.user.toLowerCase());
        ...
        const matchStatus =
            !filters.status ||
            logStatus.includes(filters.status.toLowerCase());
    ...
    // Helper pour formatter le JSON ou texte
    ...
    // Calculer les statistiques affichées (globales ou filtrées)
    const displayStats = {
        total:
            filters.user ||
            filters.action ||
    ...
    // Formater l'email pour l'affichage (masquer une partie si trop long)
